...
#' Get High-Frequency (HF) Ledger Implementation
#'
#' This asynchronous function retrieves the ledger records for high-frequency trading
#' accounts (hf ledgers) from the KuCoin API. It sends a GET request to the
#' `/api/v1/hf/accounts/ledgers` endpoint with optional query parameters.
#'
#' @param config A list containing API configuration parameters.
#' @param query A named list of query parameters to filter the ledger records. Supported parameters include:
#'   - **currency** (string, optional): One or more currencies (up to 10) to filter by.
#'   - **direction** (string, optional): "in" or "out".
...
#'   - **limit** (integer, optional): Number of results per page (default is 100; min 1, max 200).
#'   - **startAt** (integer, optional): Start time (in milliseconds).
#'   - **endAt** (integer, optional): End time (in milliseconds).
#'
#' @return A promise that resolves to a data.table containing an array of ledger records.
#'
#' @details
#' **Endpoint:** `GET https://api.kucoin.com/api/v1/hf/accounts/ledgers`
#'
#' For further details, refer to the
#' [KuCoin API Documentation](https://www.kucoin.com/docs-new/rest/account-info/account-funding/get-account-ledgers-tradehf).
#'
#' @examples
#' \dontrun{
#'     query <- list(currency = "BTC", direction = "in", bizType = "TRANSFER", lastId = 254062248624417, limit = 100, startAt = 1728663338000, endAt = 1728692138000)
#'     coro::run(function() {
#'         dt <- await(get_spot_hf_ledger_impl(config, query))
#'         print(dt)
#'     })
#' }
#'
#' @export
get_spot_hf_ledger_impl <- coro::async(function(config, query = list()) {
    tryCatch({
        base_url <- get_base_url(config)
        ...
        qs <- build_query(query)
        full_endpoint <- paste0(endpoint, qs)
        # Build headers over the full endpoint (including query string)
        headers <- await(build_headers(method, full_endpoint, body, config))
        url <- paste0(base_url, full_endpoint)
        cat("Final URL for HF ledger: ", url, "\n")  # Debug print
        response <- GET(url, headers, timeout(3))
        data <- process_kucoin_response(response, url)
        dt <- as.data.table(data)
        return(dt)
    }, error = function(e) {
        abort(paste("Error in get_spot_hf_ledger_impl:", conditionMessage(e)))
    })
})