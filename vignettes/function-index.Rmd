...
## Module: `./R/impl_spottrading_orders_add_order.R`

### Function: `add_order_impl`

**Description**: Places a new order (limit or market) to the KuCoin Spot trading system asynchronously. Before use, ensure you have sufficient funds and have retrieved symbol details for precision requirements.
...
- `keys`: List of API configuration parameters (default: `get_api_keys()`):
  ...
  - `key_version` (character): API key version (e.g., `"2"`).
- `base_url`: Character string, the base URL for the API (default: `get_base_url()`).
- `type`: Character string, order type: `"limit"` or `"market"`. **Required**.
- `symbol`: Character string, trading pair (e.g., `"BTC-USDT"`). **Required**.
- `side`: Character string, order side: `"buy"` or `"sell"`. **Required**.
- `clientOid`: Character string, unique client order ID (max 40 characters, alphanumeric with underscores/hyphens). Optional.
- `price`: Numeric, price for limit orders. Required for `"limit"`.
- `size`: Numeric, quantity for limit or market orders. Required for `"limit"`, optional for `"market"` if `funds` is provided.
- `funds`: Numeric, funds for market orders. Optional for `"market"`, mutually exclusive with `size`.
- `stp`: Character string, self-trade prevention: `"CN"`, `"CO"`, `"CB"`, or `"DC"`. Optional.
- `tags`: Character string, order tag (max 20 ASCII characters). Optional.
- `remark`: Character string, order remarks (max 20 ASCII characters). Optional.
- `timeInForce`: Character string, time-in-force: `"GTC"`, `"GTT"`, `"IOC"`, or `"FOK"` (default: `"GTC"`). Optional.
- `cancelAfter`: Numeric, cancel after n seconds (for `"GTT"`). Optional.
- `postOnly`: Logical, passive order flag (default: `FALSE`). Optional.
- `hidden`: Logical, hide order from order book (default: `FALSE`). Optional.
- `iceberg`: Logical, iceberg order flag (default: `FALSE`). Optional.
- `visibleSize`: Numeric, max visible quantity for iceberg orders. Optional, required if `iceberg` is `TRUE`.

**Returns**: A promise resolving to a `data.table` with columns:
- `orderId` (character): Unique order ID generated by KuCoin.
- `clientOid` (character): Client-specified order ID (or `NA` if not provided).
